...
±±ºPrograma  ³ RGENJ03  ºAutor  ³Fabio Simonetti     º Data ³  26/04/14   º±±
...
If lJob
     ...
     ConOut(Repl("-",100))
     ConOut(Padc("Iniciando - Verificando interface do PDV (RGENJ03) ==> "+Dtoc(Date())+" "+Time(),100))
     ConOut(Repl("-",100))
     ...
     //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
     ConOut(Repl("-",100))
     ConOut(Padc("Finalizando Job Verificando interface do PDV (RGENJ03) ==> "+Dtoc(Date())+" "+Time(),100))
     ConOut(Repl("-",100))
     RESET ENVIRONMENT
Else
     //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
     ...
     //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
     AADD(aSays,OemToAnsi("Executa o envio de informações dos região do Protheus (RGENJ03) ==> " ))
...
dbSelectarea("SX5")
SX5->(dbSeek(xFilial("SX5")+"T3"))
                                                   
While SX5->X5_TABELA == "T3"
	
	cReg := SX5->X5_CHAVE
	cSeg1 := SX5->X5_DESCRI
	
	HttpsPost("https://api.parse.com/1/classes/segment/","", "", "", "", '{"segment_type":"1","segment":"'+cReg+'","segment_description":"'+cSeg1+'"}',120,aHeadOut,@cHeadRet)
...
dbSelectarea("SX5")
SX5->(dbSeek(xFilial("SX5")+"U1"))
                                                   
While SX5->X5_TABELA == "U1"
	
	cReg := SX5->X5_CHAVE
	cSeg1 := SX5->X5_DESCRI
	
	HttpsPost("https://api.parse.com/1/classes/segment/","", "", "", "", '{"segment_type":"2","segment":"'+cReg+'","segment_description":"'+cSeg1+'"}',120,aHeadOut,@cHeadRet)

	SX5->(dbSkip())
Enddo

dbSelectarea("SX5")
SX5->(dbSeek(xFilial("SX5")+"U2"))
                                                   
While SX5->X5_TABELA == "U2"
	
	cReg := SX5->X5_CHAVE
	cSeg1 := SX5->X5_DESCRI
	
	HttpsPost("https://api.parse.com/1/classes/segment/","", "", "", "", '{"segment_type":"3","segment":"'+cReg+'","segment_description":"'+cSeg1+'"}',120,aHeadOut,@cHeadRet)
