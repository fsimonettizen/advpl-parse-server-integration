...
±±ºPrograma  ³ RFATJ01  ºAutor  ³Fabio Simonetti     º Data ³  17/04/14   º±±
...
If lJob
     ...
     ConOut(Repl("-",100))
     ConOut(Padc("Iniciando - Verificando interface do PDV (RGENJ01) ==> "+Dtoc(Date())+" "+Time(),100))
     ConOut(Repl("-",100))
     ...
     //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
     ConOut(Repl("-",100))
     ConOut(Padc("Finalizando Job Verificando interface do PDV (RGENJ01) ==> "+Dtoc(Date())+" "+Time(),100))
     ConOut(Repl("-",100))
     RESET ENVIRONMENT
Else
     //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
     ...
     //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
     AADD(aSays,OemToAnsi("Executa o envio de informações dos estados do Protheus (RGENJ01) ==> " ))
...
While SX5->X5_TABELA == "12"
	
	cEstado := SX5->X5_CHAVE
	cDesc := Capital(SX5->X5_DESCRI)
	
	HttpsPost("https://api.parse.com/1/classes/br_states/","", "", "", "", '{"state":"'+cEstado+'","state_description":"'+cDesc+'"}',120,aHeadOut,@cHeadRet)
